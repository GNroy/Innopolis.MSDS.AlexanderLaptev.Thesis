...
class Miner:
    ...
    def compute_candidate_stars(self, verbose=False):
        ...
        for label in np.sort(self._timestamps['trajectory_id'].unique()):
            ...
            candidates_df = self._timestamps[(self._timestamps['trajectory_id']>label) & \
                                            (self._timestamps['datetime'].isin(label_df['datetime'])) & \
                                            (self._timestamps['cluster'].isin(label_df['cluster'])) \
                                            ].groupby(['datetime', 'cluster']).filter(lambda x: x.name in tuple_list)[['trajectory_id', 'datetime']]
            star = []
    ...
    def save_connection_rate(self, filename):
        np.savez(filename, labels=self._connection_rate['labels'], data=self._connection_rate[matrix].data, 
                indices=self._connection_rate[matrix].indices, indptr=self._connection_rate[matrix].indptr, shape=self._connection_rate[matrix].shape)
